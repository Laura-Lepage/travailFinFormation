...
import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
        ...
        // Fonction pour ajouter un abonné à Mailchimp
        async function addSubscriber(email, firstName, lastName) {
            const apiKey = "6732faf583feefd99cfd480cc5b713cb-us13";
            const serverPrefix = "us13"; // Par exemple, "us1"
            const listId = "23d2f8ed2e";
            ...
            try {
                ...
                if (response.ok) {
                    console.log(`Successfully added ${email} to the Mailchimp list.`);
                } else {
                    const data = await response.json();
                    throw new Error(`Failed to add subscriber: ${data.title}`);
                }
            } catch (error) {
        ...
        document.querySelector('#signupForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Ajouter l'utilisateur à Mailchimp
